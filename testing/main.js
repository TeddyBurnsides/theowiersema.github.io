...
// start the magic!
window.onload = () => {
	let markov,startWord
	document.querySelector('#build').setAttribute('disabled',''); // don't generate w/o initializing first
	document.addEventListener('click', event => {
		const element=event.target; // HTML node that is clicked
		if (element.id == 'init') {
			markov = initialize(input); // generate markov array
			document.querySelector('#build').removeAttribute('disabled'); // allow generating
			document.querySelector('#init').setAttribute('disabled',''); // done initializing
		}
		if (element.id == 'build') {
			startWord=getRandomWord(input); // Choose starting word randomly
			buildSentence(markov,startWord,35); // build sentence
		}
	}, false);
}
// generate sentence from a markov array given a starting word
const buildSentence = (markov,startWord,maxValue) => {
	...
	sentenceDIV.innerHTML = sentenceDIV.innerHTML.slice(0,-1) + '.'; // remove last space + add a period
}
